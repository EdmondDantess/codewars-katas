function idBestUsers() {
    const res = []
    const monthes = [...arguments]
    const mapIds = new Map()
    monthes.forEach(el => el.forEach(innerEl => {
        mapIds.set(innerEl, 0)
    }))
    const obj = Object.fromEntries(mapIds)
    const newObj = {}
    for (const objKey in obj) {
        for (let i = 0; i < monthes.length; i++) {
            monthes[i].includes(objKey) && obj[objKey]++
        }
        obj[objKey] === monthes.length
            ? obj[objKey] = 0
            : delete obj[objKey]
    }
    for (const objKey in obj) {
        for (let i = 0; i < monthes.length; i++) {
            for (let j = 0; j < monthes[i].length; j++) {
                monthes[i][j] === objKey
                    ? obj[objKey]++
                    : null
            }
        }
        newObj[obj[objKey]] = newObj[obj[objKey]] ? [...newObj[obj[objKey]], objKey] : [objKey]
    }
    for (const newObjKey in newObj) {
        res.push([+newObjKey, newObj[newObjKey]])
    }
    res.sort((a, b) => b[0] - a[0])
    return res.map(el =>
        [el[0], el[1].sort((a, b) => a.localeCompare(b))]
    )
}
